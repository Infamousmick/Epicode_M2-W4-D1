...
function searchJob(impiego, posizione) {
  ...
  jobs.forEach((job) => {
    if (
      ...
      job.location.toLowerCase().includes(posizione.toLowerCase())
    ) {
...
function getInput() {
  let input1 = document.querySelector("#job").value;
  let input2 = document.querySelector("#location").value;
  if (input1 == "" || input2 == "") {
    alert("Inserisci entrambi gli input!");
  } else {
    searchJob(input1, input2);
  }
}
